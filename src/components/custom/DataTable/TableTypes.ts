...
// Re-export tanstack types for external use
export type { Row, RowData, Table, SortingState };
...
export interface IDataColumnMeta<TData extends RowData> {
  ...
  imageAltKey?: string;
  imageRenderer?: (imageUrl: string, row: any) => React.ReactNode;
}
...
export interface IDataTableCallbacks<TData extends RowData> {
  ...
  // called after a row drag-and-drop reorder completes
  onRowReorder?: (params: {
    activeIndex: number;
    overIndex: number;
    activeItem: TData;
...
export interface IDataTableInitialState<_TData extends RowData> {
  sorting?: Array<{
    id: string;
    desc: boolean;
  }>;
...
export interface IDataTableProps<TData extends RowData>
